    ...
    <div id="searchpage-container">
      ...
      <div id="search-form-container">
        <form onSubmit={async (event) => {
          ...
          <input type="search" id="projectsearchquery" name="projectsearchquery" placeholder="Enter query or leave blank" value={searchOptions.query} onChange={(e) => {
...
async function runQuery(searchOptions) {
  ...
  const countryFormatted = (searchOptions.country) ? ('country:' + searchOptions.country) : '';
  const themeFormatted = (searchOptions.theme) ? ('theme:' + searchOptions.theme) : '';
  // If both filters are set, add a comma inbetween
  if (countryFormatted && themeFormatted) {
    filtersFormatted = '&filter=' + countryFormatted + ',' + themeFormatted;
  // If only one filter is set, don't bother with the comma
  } else if (countryFormatted || themeFormatted) {
    filtersFormatted = '&filter=' + countryFormatted + themeFormatted;
  }
  const response = await fetch(`${API_URL}?api_key=${API_KEY}${queryFormatted}${filtersFormatted}`, {
      method: "GET",
      mode: "cors",
      headers: {
...
async function fetchProjectSearch() {
  ...
  let jsonBlob = await response.json();
  console.log(jsonBlob.themes.theme);
  return jsonBlob.themes.theme;
}

export { fetchProjectSearch };